...
const mixFloat32 = require('./mix-float32');
...
function mixAudio(audios, multiplier) {
    const first = audios[0];
    ...
    for(let i = 0; i < channels; i++) {
        const channel = audios.map((a) => a.channelData[i]);
        merged.channelData[i] = mixFloat32(channel, multiplier || 1);
    }
...
function wavAudioSprite(files, options = {}) {
    return new Promise((resolve, reject) => {
        ...
        files.forEach((filename) => {
            ...
            if (options.padding && typeof options.padding === 'number') {
                ...
                // will start samples on rounded seconds
                padding = Math.ceil(duration / 1000) * 1000 - duration + 1000;
            }
        ...
        const performMix = options.mixer || false
        const mixMultiplier = options.mixMultiplier || 1

        const merged = performMix ? mixAudio(audioData, mixMultiplier) : mergeAudio(audioData);
